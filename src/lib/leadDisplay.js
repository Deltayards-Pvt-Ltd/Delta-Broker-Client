...
export function statusColor(status) {
  const raw = typeof status === "string" ? "" : status?.color;
...
export function statusBadgeColors(status) {
  ...
  const fromCrm = typeof status === "object" ? statusColor(status) : "";
  if (fromCrm && fromCrm !== FALLBACK_STATUS_COLOR) {
    return { bg: fromCrm, text: "#FFFFFF" };
  }
  const hit = STATUS_TONES.find((t) => t.test.test(name));
  if (hit) return { bg: hit.bg, text: hit.text };
  return name ? hashTone(name) : { bg: FALLBACK_STATUS_COLOR, text: "#FFFFFF" };
}
